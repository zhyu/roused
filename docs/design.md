...
`SIGQUIT` retains Pingora's graceful-upgrade path, closes the same admission
gate when received, and runs the same cleanup after Pingora broadcasts
shutdown; it is not a Roused reload protocol.
...
Pingora's grace period is fixed at 20 seconds and its final runtime shutdown
timeout is configured to two seconds. Generated gateway plists set integer
`ExitTimeOut=30` so launchd permits the complete bounded process-exit path to
finish. A crash, `SIGKILL`, or another unhandled terminating signal cannot
perform cleanup. A replacement gateway creates fresh lifecycle state for all
configured labels and therefore adopts a surviving target with a fresh idle
timeout; post-restart activity resets that new timeout. A deliberate restart
is stop-then-start: the old gateway cannot distinguish it from permanent stop,
cleans quiescent targets, and the replacement does not eagerly wake them.
