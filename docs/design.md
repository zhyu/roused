...
5. allow the target to receive SIGTERM after the service is idle and an
   optional external command says stopping is safe; and
6. make one bounded, conservative cleanup pass over quiescent configured
   targets when the gateway deliberately terminates.
...
The gateway treats `SIGTERM` and `SIGINT` as coordinated termination. It closes
a request-admission gate at the earliest proxy-context boundary, then allows at
most five seconds for pre-lease admission, in-flight service leases, and wake
commands to drain. Each independently quiescent service is handled
concurrently; a busy service does not block cleanup of another service. New or
buffered requests that cross the closed gate receive a deterministic `503`
shutdown response and cannot acquire a lifecycle lease or initiate a wake.
`SIGQUIT` closes the same admission gate and completes the same cleanup before
entering Pingora's graceful-upgrade path; it is not a Roused reload protocol.

For every configured label that becomes quiescent within that window, shutdown
runs one stop decision without applying the ordinary idle timeout or checker
retry cooldown. It still honors `can_stop_command`, does not duplicate a stop
attempt already in progress, and atomically rechecks request generation,
...
target running. Cleanup considers every configured label; Roused does not
persist provenance proving which gateway instance woke a job. There is no
target PID discovery, process-exit wait, escalation, or target-specific drain
protocol.

The signal watcher requests cleanup and waits for its completion before
returning `SIGTERM`, `SIGINT`, or `SIGQUIT` to Pingora. That coordination wait
has a 20-second fail-safe so an internal cleanup defect cannot block gateway
termination indefinitely. Pingora's subsequent grace period is fixed at one
second and its final runtime shutdown timeout is zero seconds. Thus normal
termination takes the actual cleanup duration plus about one second. A request
or checker timeout ordinarily produces about a six-second exit; the bounded
five-second drain, five-second checker, five-second launchctl call, and
one-second timed-out-child cleanup can make the longest legitimate sequential
service path approach 16 seconds before Pingora teardown. Configured services
remain concurrent, so their number does not multiply that bound. A request
that outlives the five-second drain deadline may be interrupted during the
following Pingora grace period.

Pingora's `SIGQUIT` path adds its fixed five-second graceful-upgrade delay after
Roused cleanup and before the one-second grace period. Generated gateway plists
set integer `ExitTimeOut=30` as a launchd safety ceiling for these bounded
paths; it does not delay an earlier exit. A crash, `SIGKILL`, or another
unhandled terminating signal cannot perform cleanup. A replacement gateway
creates fresh lifecycle state for all configured labels and therefore adopts a
surviving target with a fresh idle timeout; post-restart activity resets that
new timeout. A deliberate restart is stop-then-start: the old gateway cannot
distinguish it from permanent stop, cleans quiescent targets, and the
replacement does not eagerly wake them.

The gateway plist generated by the binary defines a current-user LaunchAgent
with `RunAtLoad=true`, `KeepAlive=true`, and `ExitTimeOut=30`; the operator lints
and bootstraps it manually. The MVP never uses root, sudo, a privileged broker,
...
- target-specific graceful drain protocols, SIGKILL escalation, PID discovery,
  and repair of a target's restart policy; and
